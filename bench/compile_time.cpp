// Compile-time budget subject (spec §13.7): a 50-member struct and a 5-deep nest, each
// encoded and decoded with vcodec. CompileTimeBudget.cmake times the compilation of this
// file and compares it with compile_time_baseline.txt; the bench_compile_time target also
// builds it as a program so the measured code is known to link and run.
...
int main() {
    ...
    std::string const wj = vcodec::json::encode(w);
    std::string const nj = vcodec::json::encode(nest);
    auto const wr = vcodec::json::decode<Wide>(wj);
    auto const nr = vcodec::json::decode<L1>(nj);
    std::printf("wide: %zu bytes, decode %s\n", wj.size(), wr ? "ok" : "failed");
    std::printf("nest: %zu bytes, decode %s\n", nj.size(), nr ? "ok" : "failed");
    return (wr && nr) ? 0 : 1;
}
